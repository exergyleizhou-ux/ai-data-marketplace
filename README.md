...
## 🔬 可验证证据层（Compute-to-Data）· Verifiable-Evidence Layer

> 在你看不到的数据上跑一个审计过的算法（断网沙箱），拿到一张**任何人都能重算核验**的证书——把**这次输出**钉死到**这个 digest 锁定的算法镜像**和**这份数据集**。诚实主张是**溯源**,不是隐私。

这是本仓库的技术内核,**已在真实公开数据上端到端验证**(UCI Wine Quality):
- 完整性筛查 → 证书 `VO-0C300E9FAED2`(下载结果重算 SHA-256 == 证书,公开 `/verify` 可核验)
- 因果 ATE(alcohol→quality)→ 证书 `VO-2B9CCA235122`
- 反恶意算法输出闸 · 真 Intel TDX quote 离线验证 · L2 fail-closed · 中心化 DP,全部 TDD。

**完整技术展示(给 skeptic 读的)→ [`docs/技术展示-可验证证据层.md`](docs/技术展示-可验证证据层.md)**
威胁模型与诚实边界 → [`docs/威胁模型与保证-C2D可验证证据层.md`](docs/威胁模型与保证-C2D可验证证据层.md) ·
独立复现 → [`docs/复现-独立验证C2D结果.md`](docs/复现-独立验证C2D结果.md)
