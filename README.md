...
迁移文件在 [`backend/migrations/`](backend/migrations/)（000001..000003）并编译进二进制。`make help` 列出全部目标。
...
- **[H1]** Stripe 连接账户落库：`sellerID→acct` 映射从内存缓存改持久化到 `payout_account` 表（auth 模块）。
- **[H2]** 退款/分账回退：`order.ResolveDispute` 选「退款」时补 Stripe Refund + Transfer Reversal。
- **[H3]** 结算可靠性：outbox 表 + 分布式锁（PG advisory lock / Redis）+ 重试。
- **[H4]** 刷新令牌吊销：Redis denylist（auth/token）。
- **[H5]** 前端真实支付 UI：接 Stripe.js，用 `pk_test` 在结账页真刷卡。
- **[H7]** 更多集成测试 / CI 加 `-race`。
