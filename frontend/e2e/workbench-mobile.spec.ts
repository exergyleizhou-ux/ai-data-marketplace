...
test("390px viewport exposes runtime status and cancellation without overflow", async ({ page }) => {
  ...
  await expect(page.getByRole("dialog")).toBeVisible(); await page.getByRole("button", { name: /取消 Run/ }).click(); await expect.poll(async () => (await (await page.request.get(`/api/lumen/code/v1/runs/${active.runID}`)).json()).run.status).toBe("canceled"); await workbench.publishCodeSnapshot(active.runID); await expect(page.getByRole("button", { name: "运行详情", exact: true })).toContainText("canceled");
