...
test("authenticated runtime reload recovers the same durable Run and events", async ({ page }) => {
  ...
  const completed = await workbench.runCode("E2E_RECOVERY complete this controlled request"); await workbench.publishCodeSnapshot(completed.runID);
  ...
  await page.getByRole("button", { name: "关闭运行详情" }).click(); await page.reload(); await expect(page.locator("iframe")).toBeVisible(); await workbench.publishCodeSnapshot(runID!);
...
test("cancel action terminates an actual active Code Run", async ({ page }) => {
  ...
  await expect.poll(async () => (await (await page.request.get(`/api/lumen/code/v1/runs/${runID}`)).json()).run.status).toBe("canceled"); await workbench.publishCodeSnapshot(runID!); await expect(page.getByRole("button", { name: "运行详情", exact: true })).toContainText("canceled");
});
