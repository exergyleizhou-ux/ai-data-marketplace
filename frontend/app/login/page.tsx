...
import { Alert, AuthShell, Button, Field, Input } from "@/components/ui";
...
export default function LoginPage() {
  ...
  // Already signed in → don't show the form; send them into the app.
  useEffect(() => {
    if (user) router.replace("/datasets");
  }, [user, router]);
  ...
  if (need2FA) {
    return (
      <AuthShell kicker={t("安全", "Security")} title={t("两步验证", "Two-factor")}>
  ...
  return (
    <AuthShell
      kicker={t("欢迎回来", "Welcome back")}
      title={t("登录", "Sign in")}
      footer={
        <>
          <Link href="/auth/reset" className="cue-underline hover:text-ink">{t("忘记密码?", "Forgot password?")}</Link>
          {" · "}
          <Link href="/register" className="font-medium text-ink cue-underline">{t("注册", "Sign up")}</Link>
        </>
      }
    >
