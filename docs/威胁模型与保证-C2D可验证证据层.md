...
- **失效边界(已知的大缺口)**:在**当前默认的 L1** 部署下,运营方**能**看到暂存的数据——这一点**如实披露,不掩盖**。L2 是设计上的答案,但其硬件那半受限于真实 TEE 云/硬件(`runner_tee_tdx.go` 是 scaffold + mock attester);L3 单进程已验证,真·多节点(Secretflow)受限。**在拿到真 TEE 之前,买方与卖方对 L1 的信任假设里包含「信任运营方」。**
...
|---|---|---|
| 运营方可见数据(L1) | 如实披露 | 真 TEE 云/硬件 → L2 远程证明 |
